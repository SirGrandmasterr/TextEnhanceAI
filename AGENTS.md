...
- `core/` holds editing logic and model backends (`backend.py` contract, `ollama_service.py`, `remote_service.py`, `settings.py`), manuscript splitting (`chunking.py`), the automatic review model, runner and persistence (`workflow.py`) and the edit-kind heuristics (`change_kinds.py`); `ui/` holds Tkinter screens (`app.py`, `review_panel.py`, `connection_dialog.py`, `workflow_screen.py`) and the shared `theme.py`. Keep Tk out of `core/` so it stays testable.
