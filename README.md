...
4. **Choose the evaluation mode.** *One combined request per segment (faster)* asks the model once per segment for a JSON list of every edit with its category (spelling, grammar, expression) and reason — about **1 request per segment**. *Three separate requests (more thorough)* runs each check on the whole segment on its own — **3 edit requests per segment** — and explains the changes afterwards: trivial fixes (spacing, punctuation, capitalization, one-letter typos) get a fixed sentence in the text's language without asking the model, and the remaining changes are explained in batches of up to 6 segments per check and request, so explanations add far fewer than one request per check. Combined mode uses the server's JSON-schema output when available; if a server rejects it, or an answer cannot be matched back to the text, that segment is evaluated with the separate requests automatically (the report lists each result's method).
5. **Start.** The evaluation runs in the background with the parallelism you chose; segments become reviewable as soon as their checks are done, and you can pause, resume, close the app and come back later — everything is saved in `<name>.teai/project.json` next to the manuscript.
6. **Review.** The left tree lists chapters and segments with their state. The right pane has three tabs: *Segment* (below), *Chapter* — the whole chapter as it currently reads, applied changes underlined green, pending ones on amber, rejected struck through, flagged ones in amber; click a passage to open its change in the Segment tab — and *All changes*, a table of every pending change in the project with check/kind/text filters, multi-select with *Accept selected* / *Reject selected* (or `Alt+A` / `Alt+R`; one undo step) and double-click to jump to the segment. In the Segment tab the segment text highlights every proposed change in the colour of its check; below it, one card per change shows the before/after diff, the model's short explanation and Accept/Reject buttons. Filters show or hide checks, *Accept all shown* / *Reject all shown* handle a segment at once, and *Preview result* renders the segment with the current decisions. Every card carries a grey tag with the *kind* of edit (whitespace, punctuation, capitalization, spelling, word choice, insertion, deletion, rewrite); the *Kinds ▾* menu hides kinds you do not want to see (a view filter only — hidden changes still count and are still applied if accepted, and the choice is saved with the project) and offers *Accept all…* / *Reject all…* per kind across the whole project, confirmed with the count and undoable in one step. *Undo* (`Alt+Z`) reverts the last accept or reject — a bulk action (*Accept all shown*, *Reject all shown*, *Reject flagged*) is reverted as a whole — and jumps to the change it restored; *Decisions...* lists every decision made so far (newest first) with double-click to jump to the change. *Statistics...* shows per-chapter rows (words, changes, changes per 1,000 words, accepted/rejected/pending), per-check and per-kind totals with acceptance rates and the 20 most frequent corrections (double-click opens the first occurrence); *Copy as Markdown* puts the same tables on the clipboard, and the export appends them to `report.md` under *Statistics*. If you edit the manuscript file while a review is open, *Check source* (also run when a project is opened) notices it and offers a **re-sync**: the file is split again, segments whose text is unchanged keep their results and decisions, new or edited segments are queued for evaluation, and pending or accepted changes of segments that disappeared are written to `resync-<timestamp>.md` in the project folder so nothing is lost silently. Once every segment has been evaluated, *Consistency...* runs a chapter-spanning check: names spelled two ways (`Nyxara`/`Nixara`), a compound with and without a hyphen (`E-Mail`/`Email`), numbers 0–20 as digits and as words, mixed quotation-mark styles, and chapters whose point of view or tense deviates from the rest by more than 25 points. The deterministic candidates appear at once in the *Consistency* tab (unverified); the model then triages the name, hyphenation and number groups in batches (issue / intentional, preferred form, reason). Double-click an occurrence to open it, *Dismiss* hides a finding, and *Apply preferred everywhere* writes the preferred form over every other variant as your own corrections — one undo step. Findings are saved with the project and recomputed after a re-sync. The log is saved with the project, so undo still works after reopening it. *Re-evaluate* in the segment header (or right-click a row in the tree → *Evaluate again* → all checks, one check, or the whole chapter) discards those results and their decisions and runs the checks again immediately — while an evaluation is running the tasks join the queue, otherwise it starts one; the old decisions stay in the log greyed out and are skipped by undo.
7. **Export.** Writes `reviewed/NN - Title.txt` per chapter, `<name>-reviewed.txt` for the whole manuscript, and `report.md` listing every change, its decision and explanation.
