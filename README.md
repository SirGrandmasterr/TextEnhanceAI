...
Keyboard shortcuts in the review: `Alt+A` / `Alt+R` accept or reject the selected change (and move to the next pending one), `Alt+↑` / `Alt+↓` select the previous or next change, `Alt+←` / `Alt+→` switch segments.
