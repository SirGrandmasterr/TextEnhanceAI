...
3. **Author's instructions (optional).** Standing rules every check must respect, one per line, e.g. `British spelling`, `keep dialect inside dialogue`, `never touch quotations`. They are appended to every request and to the explanation prompt and take precedence over the built-in rules where they conflict; the last used set pre-fills the next project. On the project screen, *Options...* edits them later and offers to re-evaluate all segments (existing decisions are lost).
